...
def test_load_configuration_secrets_files(tmp_path):
    # shutil.copy(CONFIG_PATH, tmp_path / "config.toml")
    def _get_jira_and_instances(config_dict: dict[str, Any]) -> list[tuple[str, Any]]:
        return list(config_dict["instances"].items()) + [
            ("jira", config_dict["general"]["jira"])
        ]
    ...
    # instances = ["github.com", "gitlab.com", "codeberg.org", "pagure.io"]
    for instance_name in config_toml["instances"].keys():
        shutil.copy(
            PROJECT_ROOT / f"{instance_name.replace('.', '')}_jira_usermap.toml",
            tmp_path,
        )
